...
class UserRedux extends Component {
    ...
    handleOnChangeImage = async (event) => {
        ...
        let file = data[0];
        if (file) {
            let base64 = await CommonUtils.getBase64(file); //--> Mã hóa base64
    ...
    checkValidateInput = () => {
        ...
        for (let i = 0; i < arrCheck.length; i++) {
            if (!this.state[arrCheck[i]]) {
                isValid = false;
                alert(`Input's required: ` + arrCheck[i])
                break;
            }
        }
        return isValid;
    ...
    handleEditUserFromParent = (user) => {
        let imageBase64 = '';
        if (user.image) {
            imageBase64 = new Buffer(user.image, 'base64').toString('binary'); //--> Giải mã base64
        }
        this.setState({
            email: user.email,
            password: 'hardCode',
            ...
            <div className='user-redux-container'>
                ...
                <div className='user-redux-body'>
                    <div className='container'>
                        <div className='row'>
                            <div style={{ fontSize: '20px' }} className='col-12 my-3 mt-3'><FormattedMessage id="manage-user.add" /></div>
